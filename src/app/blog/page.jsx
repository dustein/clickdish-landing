  ...
  return (
    <main className="max-w-4xl mx-auto">
      <div className="text-center mb-10">
        <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-clickdish-magenta to-clickdish-orange text-transparent bg-clip-text mb-4 md:mb-6">Saúde e Fitness</h1>
        <p className="text-sm md:text-lg text-gray-600">Dicas práticas, receitas e tudo que você precisa saber para manter o corpo e a alma leves.</p>
      </div>
        ...
        {posts.map((post) => (
          <article key={post.slug} className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 p-8 hover:shadow-[0_8px_30px_rgb(236,64,122,0.1)] transition-all duration-300">
            <Link href={`/blog/${post.slug}`} className="block group">
              <div className="flex items-center gap-3 text-xs md:text-sm text-clickdish-magenta font-bold mb-4">
              ...
              <h2 className="text-xl md:text-2xl font-bold text-gray-900 group-hover:text-clickdish-orange transition-colors mb-4">{post.title}</h2>
